...
class MenuItem implements ArrayableContract
{
    ...
    /**
     * Set the Key property when the key is defined in the link attributes.
     *
     * @param array $properties
     *
     * @return array
     */
    protected static function setKeyAttribute(array $properties)
    {
        /* Key */
        $key = Arr::get($properties, 'attributes.key');
        if (!is_null($key)){
            $properties['key'] = $key;

            Arr::forget($properties, 'attributes.key');
            return $properties;
        }
    ...
    /**
     * Set the Active property when the active is defined in the link attributes.
     *
     * @param array $properties
     *
     * @return array
     */
    protected static function setActiveAttribute(array $properties)
    {
        /* Active */
        $active = Arr::get($properties, 'attributes.active');
        if (!is_null($active)){
            if (isset($properties['active']) && is_array($properties['active'])){
                $properties['active'] = array_merge($properties['active'], $active);
            }else{
                if (is_array($active)){
            ...
            Arr::forget($properties, 'attributes.active');
        }
        return $properties;
    }
     ...
     * @return self
     */
    public function setActiveProperties($active)
    {
        /* Active */

        if (!is_null($active)){
            if (is_array($active) && is_array($this->active)){
                $this->active = array_merge($this->active, $active);
                $this->properties['active'] = array_merge($this->active, $active);
            }else{
                if ($this->active && is_array($this->active)){
                    $this->active = array_merge($this->active, $active);
                    $this->properties['active'] = array_merge($this->active, $active);
                }else {
                    $this->active = $active;
    ...
    public static function make(array $properties)
    {
        ...
        $properties = self::setKeyAttribute($properties);
        $properties = self::setActiveAttribute($properties);
    ...
    public function dropdown($title, \Closure $callback, $order = 0, array $attributes = array())
    {
        ...
        if(isset($attributes['url'])&&!empty($attributes['url'])){
            $properties['url'] = $attributes['url'];
        }elseif(isset($attributes['route'])&&!empty($attributes['route'])){
            $properties['route'] = $attributes['route'];
        }
    ...
    public function getAttributes()
    {
        ...
        Arr::forget($attributes, ['active', 'icon', 'key']);
