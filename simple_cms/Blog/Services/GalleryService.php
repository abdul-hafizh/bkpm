...
class GalleryService
{
    public function save_update($request)
    {
        ...
        try{
            ...
            $content = serializeCustom($request->input('content'));
            foreach ($locales as $locale) {
                $data_pages['content'][$locale] = $content;
            }
		...
		$activity_group = 'edit';
            ...
            activity_log(LOG_GALLERY, $activity_group, 'Your ' . ($update ? 'edit':'add') . ' gallery <b>'.$pages->title.'</b>', $logProperties, $pages);
            return responseMessage(__('core::message.success.save'), [
                'redirect' => route('simple_cms.blog.backend.gallery.edit', ['post_slug' => $pages->slug]),
                'link_preview' => route('simple_cms.blog.backend.gallery.preview', ['post_slug' => $pages->slug])
            ]);
        }catch (\Exception $e){
            \DB::rollback();
            \Log::error($e);
    ...
    public function change_status($request)
    {
        ...
        activity_log(LOG_GALLERY, 'change_status', 'Your change status to: <b>' . ucwords($parse_change_status[1]) . '</b>. <br/>For gallery <b>'.$post->title.'</b>', [], $post);
    ...
    public function restore($request)
    {
        ...
        if(!empty($id) OR !is_null($id)) {
            ...
            activity_log(LOG_GALLERY, 'restore', 'Your restore '.strtolower($page->type).' <strong>' . $page->title . '</strong>',[],$page);
            $page->restore();
        }
    ...
    public function soft_delete($request)
    {
        ...
        if(!empty($id) OR !is_null($id)) {
            ...
            activity_log(LOG_GALLERY, 'soft_delete', 'Your trashed '.strtolower($page->type).' <strong>' . $page->title . '</strong>',[],$page);
            $page->delete();
        }
    ...
    public function force_delete($request)
    {
        ...
        if(!empty($id) OR !is_null($id)) {
            $page = PostModel::withTrashed()->where('id', $id)->first();
            activity_log(LOG_GALLERY, 'force_delete', 'Your permanent delete '.strtolower($page->type).' <strong>' . $page->title . '</strong>',[],$page);
            $page->forceDelete();
        }
        return responseMessage('Permanent delete success');
    }

    protected function generateSlug($id, $slug, $count = 0, $original = '')
    {
