...
class Views implements ViewsContract
{
     ...
     *
     * @var \SimpleCMS\EloquentViewable\Contracts\Viewable
     */
    protected $viewable;
     ...
     *
     * @var \SimpleCMS\EloquentViewable\Support\Period|null
     */
     ...
     *
     * @var \SimpleCMS\EloquentViewable\Contracts\Visitor
     */
    protected $visitor;
     ...
     *
     * @var \SimpleCMS\EloquentViewable\CooldownManager
     */
    ...
    public function __construct(
        ...
        $this->cacheLifetime = Carbon::now()->addMinutes($config['eloquentviewable']['cache']['lifetime_in_minutes']);
     ...
     * Set the period.
     *
     * @param  \SimpleCMS\EloquentViewable\Period
     * @return $this
     */
     ...
     * Set the visitor.
     *
     * @param  \SimpleCMS\EloquentViewable\Contracts\Visitor
     */
    ...
    protected function shouldRecord(): bool
    {
        ...
        if ($this->config->get('eloquentviewable.ignore_bots') && $this->visitor->isCrawler()) {
            return false;
        }

        // If we honor to the DNT header and the current request contains the
        // DNT header, return false
        if ($this->config->get('eloquentviewable.honor_dnt', false) && $this->visitor->hasDoNotTrackHeader()) {
            return false;
        }

        if (collect($this->config->get('eloquentviewable.ignored_ip_addresses'))->contains($this->visitor->ip())) {
            return false;
        }
