<?php namespace SimpleCMS\Core\Supports;
...
class AssetContainer {
     ...
     * <code>
     *      // Add an asset to the container
     *      CoreAsset::container()->add('jquery', 'js/jquery.js');
     *
     *      // Add an asset that has dependencies on other assets
     *      CoreAsset::add('jquery', 'js/jquery.js', 'jquery-ui');
     *
     *      // Add an asset that should have attributes applied to its tags
     *      CoreAsset::add('jquery', 'js/jquery.js', null, array('defer'));
     * </code>
     *
     * @param  string  $name
     * @param  string  $source
     ...
     */
    protected function added($name, $source, $dependencies = array(), $attributes = array(), $position = 'default')
    {
        if (is_array($source))
        {
            foreach ($source as $path)
            {
                ...
                $this->added($name, $path, $dependencies, $attributes, $position);
            }
        }
        else
        {
            ...
            /* for asset script on top */
            if ( $type == 'script' && ( $name == 'scriptTop' OR $name == 'scriptsTop' ) ){
                $type = 'scriptTop';
            }

            if ( $type == 'script' && $position == 'top' ){
                $type = 'scriptTop';
            }
     ...
     * @return AssetContainer
     */
    public function add($name, $source = null, $dependencies = array(), $attributes = array(), $position = 'default')
    {
        if(!is_array($name)) {
            if(!isset($source)) throw new \ErrorException("Missing argument 2 for SimpleCMS\Core\Supports\AssetContainer::add()", 1);

            return $this->added($name, $source, $dependencies, $attributes, $position);
        }

        foreach ($name as $array) {
            if(count($array) < 2) throw new \ErrorException("Missing value 2 of the array for SimpleCMS\Core\Supports\AssetContainer::add()", 1);
            $container = $array[0];
            $source = $array[1];
            $dependencies = isset($array[2]) ? $array[2] : [];
            $attributes = isset($array[3]) ? $array[3] : [];
            $position = isset($array[4]) ? $array[4] : 'default';
            $this->add($container, $source, $dependencies, $attributes, $position);
        }
    ...
    /**
     * Add a JavaScript file to the registered assets on top.
     *
     * @param  string           $name
     * @param  string           $source
     * @param  array            $dependencies
     ...
     * @return AssetContainer
     */
    public function scriptTop($name, $source, $dependencies = array(), $attributes = array())
    {
        $this->register('scriptTop', $name, $source, $dependencies, $attributes);
    ...
    public function html($group, $source, $attributes)
    {
        ...
        switch ($group)
        {
            case 'script' :
            case 'scriptTop':
                $attributes['src'] = str_replace(['http://', 'https://'], '//', $source);
            ...
            case 'style' :
                ...
                $attributes['href'] = str_replace(['http://', 'https://'], '//', $source);
