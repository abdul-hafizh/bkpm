...
trait Translatable
{
    ...
    public function setAttribute($attribute, $value)
    {
        if ($this->isTranslatable($attribute) && !empty($value)) {
            ...
            if (is_string($value)) {
                ...
                if($json !== false && !is_null($json) && $value != $json) $value = json_decode($value, true);
            }

            /* save temp attributes */
            $this->attributes_temp[$attribute] = $value;

            if(is_array($value)){
                $this->attributes[$attribute] = $value;
                $value = $value[simple_cms_setting('locale', app('config')->get('app.locale'))];
            }
        }
     ...
     * @param $attribute
     */
    public function getRawAttributeTranslatable($attribute)
    {
        return Arr::get($this->attributes_temp, $attribute, '');
    }
