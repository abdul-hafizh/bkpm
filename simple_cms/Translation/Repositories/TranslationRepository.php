<?php namespace SimpleCMS\Translation\Repositories;
...
class TranslationRepository extends Repository
{
     ...
     *
     * @var \SimpleCMS\Translation\Models\Translation
     */
    protected $model;
    ...
    /**
     *  Constructor
     *  @param  \SimpleCMS\Translation\Models\Translation   $model  Bade model for queries.
    ...
    public function __construct(Translation $model, Application $app)
    {
        $this->model         = $model;
        $this->app           = $app;
        $this->defaultLocale = simple_cms_setting('locale', $app['config']->get('app.locale'));;
    ...
    public function updateDefaultByCode($code, $text)
    {
        ...
        if (is_array($text)){
            ...
            foreach ($texts as $locale => $text) {
                ...
                if (!$translation && !empty($text)) {
                    $this->create(compact('locale', 'namespace', 'group', 'item', 'text'));
                }else {
                    if (!empty($text)) {
                        $this->update($translation->id, $text);
                    }
                }
    ...
    public function loadArray(array $lines, $locale, $group, $namespace = '*')
    {
        ...
        foreach ($lines as $item => $text) {
            if (is_string($text)) {
                ...
                if ($translation && !$translation->isLocked() && !empty($text)) {
                    $translation->text = $text;
                    $saved             = $translation->save();
                    if ($saved && $translation->locale === $this->defaultLocale) {
    ...
    public function allByLocale($locale, $perPage = 0)
    {
        ...
        return $perPage ? $translations->paginate($perPage) : $translations->cursor();
    }
