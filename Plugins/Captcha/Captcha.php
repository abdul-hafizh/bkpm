 ...
 *
 * @copyright Copyright (c) 2020 Ahmad Windi Wijayanto
 * @version 2.x
 * @author Ahmad Windi Wijayanto
 * @contact ahmadwindiwijayanto@gmail.com
 * @web https://whendy.net
 * @date 2020-03-07
...
class Captcha
{
    ...
    public function __construct(
        ...
        $this->hasher = $hasher;
        $this->str = $str;
        $this->characters = config('plugins.captcha.characters', ['1', '2', '3', '4', '6', '7', '8', '9']);
        $this->fontsDirectory = config('plugins.captcha.fontsDirectory', __DIR__ . '/assets/fonts');
    }
    ...
    {
        if ($this->config->has('plugins.captcha.' . $config)) {
            foreach ($this->config->get('plugins.captcha.' . $config) as $key => $val) {
                $this->{$key} = $val;
            }
        }
    }
