...
class InputSurveySaveUpdateRequest extends FormRequest
{
    protected $role = [];
    protected $message = [];
    ...
    public function rules()
    {
        ...
        if ($this->input('data.profil_produk_barang_jasa')) {
            foreach ($this->input('data.profil_produk_barang_jasa') as $k => $ha) {
                if ($this->file("data.profil_produk_barang_jasa.{$k}.foto_dokumen_upload")) {
                    foreach ($this->file("data.profil_produk_barang_jasa.{$k}.foto_dokumen_upload") as $k1 => $ha1) {
                        $this->role["data.profil_produk_barang_jasa.{$k}.foto_dokumen_upload.{$k1}"] = [
                            'max:15000',
                            'mimes:jpg,jpeg,png,pdf'
                        ];
                    }
        ...
        if ($this->file('data.profil_usaha.flow_chart_proses_produksi_upload')){
            foreach ($this->file('data.profil_usaha.flow_chart_proses_produksi_upload') as $k => $ha) {
                $this->role["data.profil_usaha.flow_chart_proses_produksi_upload.{$k}"] = [
                    'max:15000',
                    'mimes:jpg,jpeg,png,pdf'
                ];
            }
    ...
    public function messages()
    {
        ...
        if ($this->input('data.profil_produk_barang_jasa')) {
            foreach ($this->input('data.profil_produk_barang_jasa') as $k => $ha) {
                if ($this->file("data.profil_produk_barang_jasa.{$k}.foto_dokumen_upload")) {
                    foreach ($this->file("data.profil_produk_barang_jasa.{$k}.foto_dokumen_upload") as $k1 => $ha1) {
                        $filename = $ha1->getClientOriginalName();
                        $this->message["data.profil_produk_barang_jasa.{$k}.foto_dokumen_upload.{$k1}"] = [
                            'max'   => Lang::get($message['max'], ['filename' => $filename]),
                            'mimes' => Lang::get($message['mimes'], ['filename' => $filename])
                        ];
                    }
        ...
        if ($this->file('data.profil_usaha.flow_chart_proses_produksi_upload')){
            foreach ($this->file('data.profil_usaha.flow_chart_proses_produksi_upload') as $k => $ha) {
                $filename = $ha->getClientOriginalName();
                $this->message["data.profil_usaha.flow_chart_proses_produksi_upload.{$k}"] = [
                    'max'   => Lang::get($message['max'], ['filename' => $filename]),
                    'mimes' => Lang::get($message['mimes'], ['filename' => $filename])
                ];
            }
