...
class SurveyUmkmMassiveImport implements OnEachRow, WithHeadingRow, WithValidation, SkipsOnError, SkipsOnFailure
{
    ...
    public function onRow(Row $row)
    {
        ...
        if ($field_complete) {
            ...
            if (isset($row['date_survey']) && !empty(trim($row['date_survey']))) {
                $data['created_at'] = parseDateImport(trim($row['date_survey']), 'Y-m-d H:i:s');
                $data['date_survey'] = parseDateImport(trim($row['date_survey']), 'Y-m-d');
            }
